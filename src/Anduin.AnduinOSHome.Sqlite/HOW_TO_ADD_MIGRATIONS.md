...
1. Navigate to the SQLite project directory:
   ```bash
   cd ./src/Anduin.AnduinOSHome.Sqlite/
   ```
...
```bash
dotnet ef migrations remove --context "SqliteContext" -s ../Anduin.AnduinOSHome/Anduin.AnduinOSHome.csproj
```

## After Creating Migrations

After creating migrations for SQLite, remember to:
1. Create the corresponding migration for MySQL (see `../Anduin.AnduinOSHome.MySql/HOW_TO_ADD_MIGRATIONS.md`)
